...
export function DebugPanel({
  ...
  const waveTimerProgress =
    nextWaveTime > 0 ? Math.min(Math.max(timeSinceLastWave / nextWaveTime, 0), 1) : 0;
  const setTimerProgress =
...
function Toggle({ label, checked, onChange, hotkey, text = null }) {
  return (
    ...
    <div className="control read-only timer-control">
      ...
      <span className="timer-value">
        ...
        <span className="value">
          {remaining.toFixed(1)}s / {total.toFixed(1)}s
        </span>
      </span>
    </div>
  );
