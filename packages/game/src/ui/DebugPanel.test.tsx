...
function createSetLullState(opts: Record<string, any> = {}) {
  const {
    setState = 'SET',
    wavesSpawned = 2,
    currentSetWaves = 5,
...
const createDefaultProps = (overrides: Record<string, any> = {}) => {
  ...
  const gameTime =
    overrides.gameTime !== undefined ? overrides.gameTime : setLullState._gameTime || sec(100);
  ...
  return {
    ...
    energyTransferCount:
      overrides.energyTransferCount !== undefined ? overrides.energyTransferCount : 10,
    ...
    onSettingChange: overrides.onSettingChange || vi.fn(),
    fps: overrides.fps !== undefined ? overrides.fps : 60,
    playerConfig:
      overrides.playerConfig !== undefined
    ...
    aiMode: overrides.aiMode || 'BEGINNER',
    onAIModeChange: overrides.onAIModeChange || vi.fn(),
  };
};
...
describe('DebugPanel', () => {
  ...
  describe('Player Tuning Section', () => {
    ...
    it('displays current slider values', () => {
      render(<DebugPanel {...createDefaultProps()} />);
      // Slider values are formatted with 1 decimal place (step >= 0.1)
      expect(screen.getByText('30.0')).toBeInTheDocument(); // waterSpeed
      expect(screen.getByText('20.0')).toBeInTheDocument(); // foamSpeed
      expect(screen.getByText('50.0')).toBeInTheDocument(); // maxPushForce
      expect(screen.getByText('30.0%')).toBeInTheDocument(); // foamSpeedPenalty (0.3 * 100)
    });

    it('shows tooltip trigger icons', () => {
      render(<DebugPanel {...createDefaultProps()} />);
      const tooltipTriggers = document.querySelectorAll('.tooltip-trigger');
      // 4 player tuning sliders + 2 playback sliders (Depth Damping, Damping Exponent)
      expect(tooltipTriggers.length).toBe(6);
    });
    ...
    it('calls onPlayerConfigChange when slider changes', () => {
      ...
      // Water Speed is the 3rd slider (after Depth Damping and Damping Exponent)
    ...
    it('converts foam penalty percentage back to decimal', () => {
      ...
      const sliders = document.querySelectorAll('input[type="range"]');
      // Foam Penalty is the 6th slider (index 5, after Depth Damping, Damping Exponent, Water Speed, Foam Speed, Push Force)
