...
export function updateAIPlayer(
  ...
  if (aiState.logTimer >= 60) {
    aiState.logTimer = 0;
    const foamInfo = bestFoam
      ? `foam@(${Math.round(bestFoam.x)},${Math.round(bestFoam.y)})`
      : 'no foam';
    console.log(
      `[AI ${aiState.mode}] ${aiState.state} pos:(${Math.round(player.x)},${Math.round(player.y)}) ${foamInfo} intensity:${foamIntensity.toFixed(2)}`
    );
  }
  ...
  if (aiState.state === AI_STATE.SEEKING) {
    ...
    const finishRide = (reason) => {
      ...
      console.log(
        `[AI ${aiState.mode}] ${reason}: ${aiState.rideTimer.toFixed(1)}s, ${Math.round(aiState.rideDistance)}px, score=${score}`
      );
...
export function drawAIKeyIndicator(ctx, input, aiState, x, y) {
  ...
  for (const k of keys) {
    ...
    ctx.fillStyle = k.pressed ? 'rgba(100, 200, 100, 0.9)' : 'rgba(50, 50, 50, 0.6)';
    ctx.beginPath();
    ctx.roundRect(kx - size / 2, ky - size / 2, size, size, 4);
    ctx.fill();

    ctx.strokeStyle = k.pressed ? 'rgba(150, 255, 150, 1)' : 'rgba(100, 100, 100, 0.8)';
    ctx.lineWidth = k.pressed ? 2 : 1;
    ctx.stroke();

    ctx.fillStyle = k.pressed ? 'rgba(255, 255, 255, 1)' : 'rgba(200, 200, 200, 0.7)';
